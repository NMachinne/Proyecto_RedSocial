...
public abstract class ADAO<T> {
	 ...
	 */
	private static final String[] database = { User.class.toString(), null, Post.class.toString(), null,
			Comment.class.toString(), null, "id", "int", "id", "int", "id_usuario", "int", "nombre", "string", "fecha",
			"string", "id_publicacion", "int", "password", "string", "texto", "string", "fecha", "string", "avatar",
			"string", "multimedia", "string", "texto", "string", null, null, "id_usuario", "int", null, null, null,
	 ...
	 * @param obj Objeto de tipo generico que puede ser solo de tipo: User, Post o
	 *            Comment
	...
	private List<Object> tObject(int i, T obj) {
		...
		case 4:
			...
			o.add((Integer) c.getUser().getId());
			o.add((Integer) c.getPost().getId());
	...
	public boolean create(T obj, String sql, Integer[] field) throws SQLException {
		...
		while (i < row) {
			...
			if (clase != null && obj.getClass().toString().equals(clase)) {
				...
				v = 1;
				while (v <= s) {
					if (field[p] != null && database[(i + field2.get(p) + 1)] != null
							&& database[(i + field2.get(p) + 1)].toString().equals("int")) {
						ps.setInt(v, (Integer) o.get(field[p]));
					} else if (field[p] != null && database[(i + field2.get(p) + 1)] != null
							&& database[(i + field2.get(p) + 1)].toString().equals("string")) {
						try {
			...
			if (clase != null && obj.getClass().toString().equals(clase)) {
				...
				while (rs.next()) {
					...
					case 4:
						...
						while (database[i] != null) {
							if (database[i].toString().equals("id_usuario")) {
								c.setUser(new User(rs.getInt(v), null, null, null));
							} else if (database[i].toString().equals("id_publicacion")) {
								c.setPost(new Post(rs.getInt(v), null, null, null));
							} else if (database[i].toString().equals("fecha")) {
